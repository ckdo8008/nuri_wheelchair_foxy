...
void MPU6050Sensor::enableBypassMode()
{
  int intPinConfig = i2c_smbus_read_byte_data(file_, INT_PIN_CFG);
  if (intPinConfig < 0) {
    reportError(errno);
    return;
  }

  // BYPASS_EN 비트를 1로 설정합니다.
  intPinConfig |= 0x02; 

  // 변경된 값을 다시 레지스터에 씁니다.
  int result = i2c_smbus_write_byte_data(file_, INT_PIN_CFG, intPinConfig);
  if (result < 0) {
    reportError(errno);
  }  
}